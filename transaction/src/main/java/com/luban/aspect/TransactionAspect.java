...
//    LbConnection lbConnection;
    ...
    public void invoke(ProceedingJoinPoint proceedingJoinPoint){
        ...
        //lbConnection.setTransaction(transaction);
        //执行本地逻辑
        try {
            //Spring 会帮我们执行mysql的事物  一直等待
           proceedingJoinPoint.proceed();
            //提交本地事物状态  ---commit
            TransactionMangage.commitTransaction(transaction,annotation.isEnd(),annotation.isStart(), TransactionType.COMMIT);
        } catch (Throwable throwable) {
            // 回滚
            TransactionMangage.commitTransaction(transaction,annotation.isEnd(),annotation.isStart(), TransactionType.ROLLBACK);
