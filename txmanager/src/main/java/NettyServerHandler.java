...
public class NettyServerHandler extends ChannelInboundHandlerAdapter {
    ...
    // 事务组是否已经接收到结束的标记
    private static Map<String, Map<String,Boolean>> isEndMap = new HashMap<String, Map<String,Boolean>>();

    private static Map<String, List<String>> transactionIdMap = new HashMap<String,List<String>>();
...
//    private static Map<String, Integer> transactionCountMap = new HashMap<String, Integer>();
    ...
    public synchronized void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        Boolean isStart = jsonObject.getBoolean("isStart");//是否是开始事务
        ...
        if ("create".equals(command)) {
            ...
            stringBooleanHashMap.put("isEnd",false);
            stringBooleanHashMap.put("isStart",false);
            isEndMap.put(groupId,stringBooleanHashMap);
            ...
            if (isEnd) {
                Map<String, Boolean> stringBooleanMap = isEndMap.get(groupId);
                stringBooleanMap.put("isEnd",true);
//                transactionCountMap.put(groupId, transactionCount);
            }
            if (isStart) {
                Map<String, Boolean> stringBooleanMap = isEndMap.get(groupId);
                stringBooleanMap.put("isStart",true);
//                transactionCountMap.put(groupId, transactionCount);
            }
            // 如果已经接收到结束事务的标记,则看是否需要回滚
            if (isEndMap.get(groupId).get("isEnd")==true
                &&isEndMap.get(groupId).get("isStart")==true) {
                JSONObject result = new JSONObject();
                result.put("groupId", groupId);
                result.put("transactionId",transactionIdMap.get(groupId));
                //commit
                //commit
                //commit
